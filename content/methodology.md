...
**Coloring — the real House outcome (Enacted exactly equals Actual).** The shapefiles are projected through the *same* `geoAlbersUsa().scale(1300).translate([487.5, 305])` as the precinct substrate, so an enacted district overlays the precincts exactly (state bounding boxes agree to ≈1 screen unit). Unlike the algorithmic maps, each enacted district is colored by the **real U.S. House result** for that cycle — the actual D–R margin of the seat, won by whoever actually won it — from the MIT Election Data & Science Lab (EDSL) U.S.-House 1976–2022 returns (`scripts/lib/house-returns.mjs`; a proper RFC-4180 parse, winner decided per *candidate* so fusion lines in NY/CT and independents are handled correctly). The Enacted **seat tally is the official documented result** — the same Clerk-of-the-House figures the dashboard's "Actual House" cell shows — so by construction **Enacted equals Actual, every cycle**. Independent-won seats (Bernie Sanders, VT at-large, 2000–2004) are counted as neither D nor R, exactly as the official tally treats them.

Validation: re-deriving each cycle's national winners purely from the per-district EDSL returns reproduces the official D/R split **exactly for all twelve cycles 2000–2022**. A handful of uncontested seats EDSL does not itemize (4 Texas seats in 2006, one each in 2016/2020) fall back to that cycle's two-party precinct lean — which, for a deep-safe uncontested district, still names the correct party, so the national totals still reconcile to the official result. **2024** per-district returns are not yet in the 1976–2022 academic dataset, so 2024 enacted districts are shaded by the two-party lean and flagged `est`; the 2024 **seat tally is still the official result** (D 215 · R 220), because the headline is pinned to the canonical table, not summed from the shaded map.

**This is a different comparison than Splitline/ReCom — deliberately.** Splitline and ReCom never held an election, so they can only be scored by the two-party *vote* (presidential, or the §3.5-modeled House vote for midterms). Enacted shows what *actually happened* on the real lines; the algorithmic maps show what a *neutral* mapmaker would have produced from the same partisan geography. The gap between Enacted and a neutral map is the structural effect of how the lines were drawn — the gerrymander — now read against the genuine historical outcome rather than a projection of it.
...
**Limitations.** Cartographic-boundary files are generalized (shoreline-clipped, ≈ 1:500 k), so an enacted outline is a faithful but not surveyor-grade boundary — adequate for visual and partisan comparison, not for litigation. Per-district returns are real for 2000–2022; the 2024 *shading* is the two-party lean (flagged) until per-district 2024 House returns enter the academic dataset, though the 2024 *tally* is the official result. The enacted record is one synthetic polygon per district (no in-app precinct→district table), so the per-district click-through *insight* card (county composition, demographics, multi-cycle profile) is disabled for Enacted; the per-district D-share panel, the map, and the headline seat tally — the comparison itself — are fully populated. `scripts/build-enacted.mjs` and `scripts/lib/house-returns.mjs` are the reference implementation.
